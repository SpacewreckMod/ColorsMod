...
namespace ColorsMod
{
    public static class Manager
    {
        ...
        {
            if (ColorsModPlugin.LoadDefaultColors)
            {
        ...
        public static void LoadSkidColors()
        {
            ...
            ColorsModPlugin.Instance!.Log.LogInfo("Skid mode activated");
        ...
        public static void SetOptions(string optionName)
        {
            switch (optionName)
            {
                case "disableSomeColors":
                    ColorsModPlugin.DisableSomeColors();
                    break;
                case "disableDefaultColors":
                    ColorsModPlugin.LoadDefaultColors = false;
                    break;
                case var a when a.StartsWith("auth"):
                    var spl = a.Split(' ').Skip(1).Aggregate((x, y) => x + " " + y);
                    ColorsModPlugin.ConfAuthor = spl;
                    break;
            }
        }
    }
